...
| Anthropic API Key | ダッシュボードの AI 分析チャット・コーチング機能（Claude Agent SDK） | サーバー側環境変数 |
| Claude Code 認証（Max プラン or API Key） | 各メンバーの Claude Code 利用（フックイベントの発火元） | 各メンバーの PC |

- **GitHub PAT** を設定すると、セッションごとのリポジトリ名・ブランチ名を自動取得し、リポジトリ分析ページでプロジェクト単位の AI 活用状況を可視化できます
- **Anthropic API Key** を設定すると、チーム概要・メンバー詳細ページの AI 分析チャットが利用可能になり、データに基づいた自然言語での質問や改善提案を受けられます
- **Claude Code の認証**（Anthropic API Key または Max プランの OAuth ログイン）は、各開発メンバーが Claude Code を利用するために必要です。Claude Code のセッションが発火させるフックイベントが、本システムのデータソースとなります

:::message
GitHub PAT は `repo` スコープ、Anthropic API Key は Claude Agent SDK の利用に必要です。各メンバーの Claude Code 認証は Max プランの OAuth ログインまたは個人の API Key のいずれかで構いません。
:::
